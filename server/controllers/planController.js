...
export const planListing = async (req, res) => {
  try {
    ...
    res.json({
      ...
      data:{plans : planData,
        selectedPlan : planSelected.plan.plan_name}
    });
  } catch (error) {
    console.log(error);
...
export const planSelect = async(req,res) => {
  try {
    ...
    let updatePlan = await store.updateOne({"store_url": req.token.store_url}, { $set: { 'plan.plan_name': planName } });
